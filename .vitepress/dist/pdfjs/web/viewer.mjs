...
const PDFViewerApplication = {
  ...
  async run(config) {
    ...
    // 自动将外部跨域 PDF 通过浏览器下载缓存为 Blob 后再加载
    let _isCrossOriginPdf = false;
    if (file) {
      try {
        const fileUrl = new URL(file, window.location.href);
        if (fileUrl.origin !== window.location.origin && (fileUrl.protocol === "http:" || fileUrl.protocol === "https:")) {
          _isCrossOriginPdf = true;
        }
      } catch {}
    }
    ...
    if (file) {
      if (_isCrossOriginPdf) {
        // 跨域 PDF：浏览器端下载缓存为 Blob，再用 Blob URL 加载
        const _originalUrl = file;
        const _statusEl = document.createElement("div");
        _statusEl.style.cssText = "position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:99999;background:#fff;color:#333;padding:24px 36px;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,.15);font:16px/1.6 sans-serif;text-align:center;max-width:80vw;";
        _statusEl.innerHTML = '<div style="margin-bottom:8px;font-size:18px;font-weight:600;">📄 正在下载 PDF...</div><div id="_pdf_status_text" style="color:#666;font-size:14px;word-break:break-all;"></div><div style="margin-top:12px;"><progress id="_pdf_progress" style="width:100%;height:6px;" value="0" max="100"></progress></div>';
        document.body.appendChild(_statusEl);
        const _statusText = document.getElementById("_pdf_status_text");
        const _progressBar = document.getElementById("_pdf_progress");
        _statusText.textContent = _originalUrl;

        const _tryFetchPdf = async (url) => {
          const resp = await fetch(url, { mode: url.startsWith("/") ? "same-origin" : "cors" });
          ...
          while (true) {
            ...
            if (total > 0) {
              ...
              _statusText.textContent = `${(received / 1024 / 1024).toFixed(1)} MB / ${(total / 1024 / 1024).toFixed(1)} MB (${pct}%)`;
            } else {
              _statusText.textContent = `已下载 ${(received / 1024 / 1024).toFixed(1)} MB`;
            }
          }
        ...
        (async () => {
          let blob;
          try {
            // 1. 先尝试直接浏览器 fetch（可能跨域成功）
            blob = await _tryFetchPdf(_originalUrl);
          } catch (directErr) {
            console.warn("Direct fetch failed, trying proxy:", directErr.message);
            try {
              // 2. 回退到服务器代理
              _statusText.textContent = "直接下载失败，正在通过代理下载...";
              blob = await _tryFetchPdf("/api/pdf-proxy?url=" + encodeURIComponent(_originalUrl));
            } catch (proxyErr) {
              console.error("Proxy fetch also failed:", proxyErr.message);
              _statusEl.innerHTML = '<div style="color:#e53e3e;font-size:18px;font-weight:600;">❌ PDF 下载失败</div>' +
                '<div style="margin:12px 0;color:#666;font-size:14px;">无法获取该 PDF 文件，请检查链接是否有效。</div>' +
                '<div style="font-size:12px;color:#999;word-break:break-all;">' + _originalUrl + '</div>' +
                '<div style="margin-top:12px;font-size:12px;color:#999;">' + directErr.message + ' / ' + proxyErr.message + '</div>';
              return;
            }
          }
          const blobUrl = URL.createObjectURL(blob);
          _statusEl.remove();
          this.open({ url: blobUrl, originalUrl: _originalUrl });
        })();
      } else {
        this.open({
          url: file
...
}
{
  ...
  var validateFileURL = function (file) {
    ...
    // });
    // throw ex;
    console.warn("Cross-origin PDF request allowed:", file);
  };
